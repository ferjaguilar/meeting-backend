...
exports.resolver = {
  Query: {
    login: async (__:void, { input }:{input:Iuser}) => userControllers.login(input),
    getStudent: async (__:void, { code }:{ code:string }) => studentControllers.getStudent(code),
  ...
  Mutation: {
    // eslint-disable-next-line max-len
    signup: async (__:void, { input }:{input:Iuser}, { user }:{user: IuserContext}) => userControllers.signup(input, user),
